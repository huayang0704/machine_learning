...
class SimpleLogisticRegression(object):
    def __init__(self, alpha, feature_num):
        ...
        self.__coef = np.random.uniform(0, 0.1, feature_num)
        self.__intercept = 0.
    ...
    def fit(self, X, y, verbose=False):
        ...
        while True:
            ...
            for tx, ty in zip(X, y):
                ...
                self.__coef += self.__alpha * delta * tx
                self.__intercept += self.__alpha * delta

            target = sum(map(
                lambda py, ty: ty * math.log(py) + (1 - ty) * math.log(1 - py),
    ...
    def __sigmoid(self, x):
        ...
        return 1. / (1 + math.exp(- np.dot(self.__coef, x) - self.__intercept))
    ...
    def _check_columns(self, X):
        ...
        """
        for x in X:
            if not isinstance(x, (list, tuple, array.array, np.ndarray)):
                return False
